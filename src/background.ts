import {FullTransaction, OrderInfo, parseOrders, parseTransactions} from "./amazon";
...
async function getAmazonInfo(numOrders: number): Promise<statusAnd.StatusAnd<FullTransaction[]>> {
  ...
  do {
    ...
    newOrders = parseOrders(ordersHtml);
    ...
    i += 10;
  } while (newOrders.length && orders.length < numOrders);
...
chrome.runtime.onMessage.addListener(async (request: any, sender: chrome.runtime.MessageSender, _sendResponse: (v: any) => void): Promise<void> => {
  ...
  const unapprovedTransactions = await ynabAPI.transactions.getTransactionsByType(budget.id, 'unapproved').then(r => r.data.transactions)
  const uncategorizedTransactions = await ynabAPI.transactions.getTransactionsByType(budget.id, 'uncategorized').then(r => r.data.transactions)
  const transactions = uniqBy([...unapprovedTransactions, ...uncategorizedTransactions], 'id');
  if (!transactions.length) {
    await chrome.runtime.sendMessage(statusAnd.error(`Found no Amazon YNAB transactions found.`));
    return;
  }

  const ynabAmazonTransactions = transactions.filter((t) => t.payee_name?.includes('Amazon'));
  if (!ynabAmazonTransactions.length) {
    await chrome.runtime.sendMessage(statusAnd.error(`Although we found ${transactions} Amazon transactions, none of them were classified as being from Amazon.`));
    return;
  }
